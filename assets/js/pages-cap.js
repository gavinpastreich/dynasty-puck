...
(function () {
  ...
  var KIND = { elcUp: 'ELC up', rfaInit: 'RFA (auction formula)', rfa: 'RFA (post-ELC)' };
  DP.KIND = KIND;
  ...
  DP.decisionHtml = function (p, opts) {
    ...
    if (d.why === 'age') {
      ...
      h += '<div class="hint">' + (d.kind === 'elcUp' ? 'ELC expires. League options: 2y $2.5M, 3y $4.0M, 4y $5.5M, 5y $7.0M, 6y $9.0M, one more ELC year at $1.75M (then UFA), or release.'
        : d.kind === 'rfa' ? 'RFA contract is up and he is under 27 on June 30: re-sign 2-6 years at 1.5x (2-4 yrs) or 1.75x (5-6 yrs) of the base; the base can\'t drop. Other teams can make offer sheets (offers or trade packages for his rights; no set compensation).'
          : 'RFA from an auction contract (BID or an RFA1 signed off one), priced with the league sheet formula: premium × current salary + add-on by length (this past summer\'s +$1M version).') + ' Surplus = projected market value − AAV over the term, discounted.</div>';
    ...
    if (ap) h += '<p class="small muted">If he reaches the 2027 UFA auction instead: ~<b>' + U.m(ap.mid) + '</b> (middle half of outcomes ' + U.m(ap.lo) + '–' + U.m(ap.hi) + ', a ' + ap.term + '-year band)' + (E.htdTeam(p) ? '; ' + esc(E.htdTeam(p)) + ' would pay ' + U.m(E.htdPrice(ap.mid)) + ' with the hometown discount' : '') + '.</p>';
    return h + '</div>';
  };
  ...
  DP.pages.cap = {
    ...
    render: function (el, hsh) {
      ...
      h += '<div class="card flush"><h2>League cap table</h2><div class="hint">Committed salary by season. Red = over the cap. Players in an IR slot in Fantrax don\'t count against the 2026-27 cap (shown as "IR"). Click a team for its chart.</div><div id="cap-league" style="padding:0 12px 12px"></div></div>';
      ...
      h += '<div class="card"><h2>Contract option calculator</h2><div class="hint">Pick a player to see every option at his next decision (ELC up, RFA re-sign, or auction) with the model\'s pick. Or price an auction-contract RFA by salary with the league formula (1yr S+1, 2yr 1.5S+1, 3yr 1.5S+2, 4yr 1.5S+3, 5yr 1.75S+4, 6yr 1.75S+5).</div>' +
        '<div class="controls"><label>Salary $M <input type="number" id="ext-s" step="0.1" min="0" value="3.0"></label><label>Formula <select id="ext-scale"><option value="ext2026">League formula (+$1M, summer 2026)</option><option value="ext2027">Sheet 2027 tab (+$1.5M)</option></select></label></div>' +
      ...
      ui.table(U.qs('#cap-league', el), {
        ...
        cols: [{ k: 't', l: 'Team', v: function (r) { return U.teamName(r.t); }, f: function (r) { return '<a href="' + U.hash('cap', null, { team: r.t }) + '"><b>' + esc(U.teamName(r.t)) + '</b></a>'; } }]
          .concat(E.YEARS.map(function (y, i) {
            return { k: 'y' + i, l: y, cls: 'num', v: function (r) { return r.c[i].total; }, f: function (r) { var v = r.c[i].total; return '<span class="' + (r.c[i].space < 0 ? 'bad' : '') + '">' + U.m(v, 1) + '</span><br><span class="small faint">' + U.m(r.c[i].space, 1) + ' of $' + r.c[i].cap + 'M</span>' + (r.c[i].ir ? '<br><span class="small faint" title="' + esc(r.c[i].irP.map(function (p) { return p.n + ' ' + U.m(p.cy[0]); }).join(', ')) + '">IR ' + U.m(r.c[i].ir, 1) + ' off</span>' : ''); }, csv: function (r) { return r.c[i].total.toFixed(2); } };
      ...
      function chart(tm) {
        ...
        U.qs('#cap-chart', el).innerHTML = C.stacked(E.YEARS.map(function (y) { return y.slice(2); }), series, { refLine: cap.map(function (y) { return y.cap; }), refLabel: 'cap', fmt: function (v) { return U.m(v); }, tickFmt: function (v) { return '$' + v + 'M'; }, title: 'Committed cap' }) +
          '<p class="small muted">' + esc(U.teamName(tm)) + ': ' + U.m(cap[0].total) + ' committed in 2026-27' + (cap[0].ir ? ' (plus ' + U.m(cap[0].ir) + ' on IR, off the cap)' : '') + ', ' + U.m(cap[1].total) + ' of $' + cap[1].cap + 'M in 2027-28 before re-signings and ELC graduations.</p>';
      }
      chart(t);
  ...
  DP.resignCall = function (p, a) {
    ...
    var E = DP.E, S = a && a.sal ? a.sal / 1e6 : p.sal26, prices = E.extPrices(S, E.initFormula()), best = { i: -1, s: 0 };
    prices.forEach(function (pr, i) {
      var sur = 0;
  ...
  DP.pages.offseason = {
    ...
    render: function (el, hsh) {
      ...
      h += '<div class="card"><h2>Decisions for summer 2027</h2><div class="hint">Players whose current deal ends after 2026-27. <b>ELC up</b>: 2y $2.5M · 3y $4.0M · 4y $5.5M · 5y $7.0M · 6y $9.0M · 1 more ELC year at $1.75M (then UFA) · release. <b>RFA (auction formula)</b>: players on BID or RFA1 deals, priced with the league sheet formula (this past summer\'s +$1M version; the sheet\'s 2027 tab used +$1.5M). <b>RFA (post-ELC)</b>: 2-6 yrs at 1.5x/1.75x of a base that can\'t drop. Hometown discount: the current owner pays 10% less than its winning bid at the auction. <b>UFA (27+)</b>: 27 or older on June 30, 2027, so no RFA rights. Model call = the option with the best projected surplus (market value − AAV, discounted); click a player for the full option table.</div><div id="os-dec"></div></div>';
      h += '<div class="grid g2" style="margin-top:14px"><div class="card"><h2>MNR graduations & ELCs</h2><div class="hint">Players who have reached, or are projected to reach, ' + E.RULES.gradSkater + ' career NHL GP (' + E.RULES.gradGoalie + ' for goalies) this season. They stay at $0 for the rest of 2026-27 and sign a $1.5M ELC this coming offseason (or are dropped). 🔒 = in an Active slot in Fantrax now: if he graduates there he can\'t be sent back to the minors (graduating in the minors he can stay down, but once promoted he can\'t return).</div><div id="os-elc"></div></div>';
      ...
      ui.table(U.qs('#os-dec', el), {
        ...
        cols: [{ k: 'n', l: 'Player', v: function (p) { return p.n; }, f: function (p) { return ui.pcell(p); } }, { k: 'gm', l: 'GM', v: function (p) { return p.gm; } },
          ...
          { k: 'auc', l: 'If auctioned', cls: 'num', title: 'Projected 2027 auction price if he is let go (and what his team would pay with the 10% hometown discount)', v: function (p) { var a = E.auctionPrice(p, 1); return a ? a.mid : 0; }, f: function (p) { var a = E.auctionPrice(p, 1); return a ? U.m(a.mid) + ' <span class="faint small">' + a.term + 'y</span>' + (E.htdTeam(p) ? '<br><span class="small good" title="Hometown discount">HTD ' + U.m(E.htdPrice(a.mid)) + '</span>' : '') : '–'; } }]
      });
      ...
      ui.table(U.qs('#os-ufa', el), {
        ...
        cols: [{ k: 'n', l: 'Player', v: function (p) { return p.n; }, f: function (p) { return ui.pcell(p); } }, { k: 'gm', l: 'Current GM', v: function (p) { return p.gm; } }, { k: 'sal', l: '2026-27', cls: 'num', v: function (p) { return p.sal26; }, f: function (p) { return ui.pill(p.ct) + ' ' + U.m(p.sal26); } },
          ...
          { k: 'htd', l: 'Hometown price', cls: 'num', title: 'What his current team would pay for the same winning bid (10% hometown discount; the bid still sets the term)', v: function (p) { return E.htdTeam(p) ? E.htdPrice(E.auctionPrice(p, 1).mid) : null; }, f: function (p) { var t = E.htdTeam(p); return t ? U.m(E.htdPrice(E.auctionPrice(p, 1).mid)) + ' <span class="faint small">' + esc(t) + '</span>' : '–'; } }]
      });
    }
  };
})();
