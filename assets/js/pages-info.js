  ...
  DP.pages.rules = {
    ...
    render: function (el) {
      ...
      h += '<div class="grid g2"><div class="card"><h2>League rules (as modeled)</h2><ul>' +
        ...
        '<li>Cap <b>$' + E.CAP + 'M</b> (2026-27; later seasons assumed flat). Contracts: BID, RFA1, ELC1 ($' + R.elcSalary + 'M), MNR ($0), FA. Minimum salary modeled at $' + R.minSalary + 'M.</li>' +
      ...
      h += '<div class="card span2"><h2>How the numbers are made</h2>' +
        ...
        '<h3>4 · Dynasty value</h3><p>For each season 2026-27 → 2032-33: projected WAR (age curves: forwards peak 25-27, D 26-29, G 27-31, faster decline after 31-33; growth capped at 1.8×) and contract cost (current deal → RFA extension at the 3-year formula price → UFA ends control; MNR at $0 until graduation, then a 2-year $1.5M ELC, then RFA). Season value = WAR − λ × (cost − $1M) ÷ $/WAR, with λ = ' + E.LAMBDA + '. Teams decline bad RFA prices, and a bad contract can be cut for 50% dead cap. Dynasty value = Σ discount<sup>year</sup> × season value, discount ' + E.DISCOUNT + '.</p>' +
        ...
        '<h3>Known limits</h3><ul><li>Projections are Fantrax\'s; the app does not re-project goals or assists.</li><li>No in-season injuries beyond the season-level noise; weekly lineups assume healthy players unless you remove them in the Matchup page.</li><li>Future caps are assumed flat; 2026-27 cap transfers ("Salary Cap: $X" in old trades) are not applied.</li><li>Six $1M BID signings aren\'t on the contract sheet and are assumed to be 1-year deals.</li></ul></div></div>';
      el.innerHTML = h;
    }
  };
