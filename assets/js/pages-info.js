...
(function () {
  ...
  DP.pages.schedule = {
    ...
    render: function (el, hsh) {
      ...
      if (tab === 'heat') {
        var hh = '<p class="small muted">Games per NHL team per fantasy week (P1–P3 = playoff weeks; week 19 spans two weeks around the February break). Darker = more games. Hover a cell for dates and back-to-backs.</p><div class="tbl-wrap"><table class="t heat"><thead><tr><th>Team</th>' + W.map(function (w, i) { return '<th class="num" title="' + esc(w.start + ' to ' + w.end) + '">' + wkLab(w, i) + '</th>'; }).join('') + '<th class="num">Tot</th><th class="num" title="Games in the three playoff weeks">PO</th></tr></thead><tbody>';
  ...
  DP.pages.rules = {
    ...
    render: function (el) {
      ...
      h += '<div class="grid g2"><div class="card"><h2>League rules (as modeled)</h2><ul>' +
        ...
        '<li><b>When an RFA contract is up:</b> if he is under 27 on June 30 he can be re-signed for 2-6 years at a 1.5x premium (2-4 yrs) or 1.75x (5-6 yrs) on the base, and the base can\'t go down (e.g. a 2-yr $2.5M deal becomes 2 yrs $3.75M or 6 yrs $15.75M; a 6-yr $9M base stays $9M × premium). Or accept an offer sheet, or drop to the auction. 27 or older: straight to the auction.</li>' +
        '<li><b>Initial-auction RFAs:</b> the first RFA deal of a 2025 auction player is priced with the contract sheet formula (S = salary): 1yr S+1.5, 2yr 1.5S+1.5, 3yr 1.5S+3, 4yr 1.5S+4, 5yr 1.75S+5, 6yr 1.75S+6 (2026 used +1.0 bases). Every RFA deal after that (including this year\'s RFA1 players) follows the regular RFA rule above (commissioner, 2026-09-26).</li>' +
        '<li><b>Offer sheets:</b> for RFAs coming off their first post-ELC deal, other teams can submit offers and trade packages for his rights; there is no set compensation.</li>' +
        '<li><b>Hometown discount:</b> an auction bid can let a team keep a player below the normal band (e.g. a 1-year-band salary on a 2-year deal). The app uses the actual terms from the contract sheet.</li>' +
        ...
        '<li><b>Dead cap:</b> dropping a BID (or ELC) contract keeps 50% of its salary for every remaining contract season; dropping an FA contract costs nothing. RFA1 deals are treated like BID (to confirm). Existing penalties are on the Contracts & Cap page.</li>' +
        '<li><b>MNR graduation:</b> career NHL regular-season GP ≥ 82 (skaters) / 41 (goalies), from the NHL API career totals (goalies included). ' + esc(R.gradNote || '') + ' The ELC is ' + (R.elcYears || 2) + ' years at $' + R.elcSalary + 'M. One not graduated by the end of the regular season stays at $0.</li>' +
